		...
		bool LoadFile(std::string Path)
		{
			...
			while ((!g_useDataFile && std::getline(file, curline)) || ((g_useDataFile) && ReadDataGetLine(data, size, cursor, curline)))
			{
				...
				if (algorithm::firstToken(curline) == "v")
				{
					...
					vpos.X = PLAT_stof(spos[0]);
					vpos.Y = PLAT_stof(spos[1]);
					vpos.Z = PLAT_stof(spos[2]);
				...
				if (algorithm::firstToken(curline) == "vt")
				{
					...
					vtex.X = PLAT_stof(stex[0]);
					vtex.Y = PLAT_stof(stex[1]);
				...
				if (algorithm::firstToken(curline) == "vn")
				{
					...
					vnor.X = PLAT_stof(snor[0]);
					vnor.Y = PLAT_stof(snor[1]);
					vnor.Z = PLAT_stof(snor[2]);
				...
				if (algorithm::firstToken(curline) == "usemtl")
				{
					...
					if (!Indices.empty() && !Vertices.empty())
					{
						...
						int i = 2;
						while(1) {
							tempMesh.MeshName = meshname + "_" + ToString(i);
		...
		bool LoadMaterials(std::string path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "Ka")
				{
					...
					tempMaterial.Ka.X = PLAT_stof(temp[0]);
					tempMaterial.Ka.Y = PLAT_stof(temp[1]);
					tempMaterial.Ka.Z = PLAT_stof(temp[2]);
				}
				// Diffuse Color
				if (algorithm::firstToken(curline) == "Kd")
				{
					...
					tempMaterial.Kd.X = PLAT_stof(temp[0]);
					tempMaterial.Kd.Y = PLAT_stof(temp[1]);
					tempMaterial.Kd.Z = PLAT_stof(temp[2]);
				}
				// Specular Color
				if (algorithm::firstToken(curline) == "Ks")
				{
					...
					tempMaterial.Ks.X = PLAT_stof(temp[0]);
					tempMaterial.Ks.Y = PLAT_stof(temp[1]);
					tempMaterial.Ks.Z = PLAT_stof(temp[2]);
				}
				// Specular Exponent
				if (algorithm::firstToken(curline) == "Ns")
				{
					tempMaterial.Ns = PLAT_stof(algorithm::tail(curline));
				}
				// Optical Density
				if (algorithm::firstToken(curline) == "Ni")
				{
					tempMaterial.Ni = PLAT_stof(algorithm::tail(curline));
				}
				// Dissolve
				if (algorithm::firstToken(curline) == "d")
				{
					tempMaterial.d = PLAT_stof(algorithm::tail(curline));
				}
				// Illumination
				if (algorithm::firstToken(curline) == "illum")
				{
					tempMaterial.illum = PLAT_stof(algorithm::tail(curline));
				}
				// Ambient Texture Map
